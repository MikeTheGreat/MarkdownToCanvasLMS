...
1245 tests, all passing, in well under a minute (~20s). The full IMSCC import pipeline (Pandoc subprocesses included) used to run once per assertion in `test_imscc_import.py`; it now runs once per test module via a module-scoped `imported_dir` fixture, and the ~125 read-only tests just assert against the shared output. Tests that need their own isolated output directory (failure-mode tests, the zip-input test, tests that mutate the fixture) still call `run_import` directly.
