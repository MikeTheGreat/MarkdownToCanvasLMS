...
from .markdown_cleanup import filter_pandoc_attrs, keep_id_and_style, repair_prose
...
def _simplify_pandoc_attrs(markdown: str) -> str:
    ...
    Classes and other key=value attributes (Canvas RCE and Word cruft, not
    meaningful outside Canvas) are dropped; `id` is kept since in-document
    anchor links may target it, and `style` is kept since it's user-authored
    formatting. A span or fenced div stripped down to nothing is unwrapped, so
    no bare `[text]` (which renders its brackets) or empty container is left.
    See markdown_cleanup.filter_pandoc_attrs().
    """
    markdown = filter_pandoc_attrs(markdown, keep_id_and_style, skip_code_blocks=False)
...
#
# relative_to says where counting starts:
#   { type = "START_OF_QUARTER" }        the term's first_day
#   { type = "FIRST_CLASS_OF_QUARTER" }  the first class day on or after first_day
#   { type = "NO_DUE_DATE" }             the item gets no due date (due_at = "NONE")
#   { type = "ASSIGNMENT", assignment_name = "Other item" }
#       the due date of another item in the same table (that item must have a
#       due date). Counting starts from its date; the time of day resets to
#       default_due_time.
#
# offsets is a list, applied in order to the date from relative_to (at default_due_time):
#   "+N CALENDAR_DAY", "-N CALENDAR_DAY"  move N calendar days
#   "+N CLASS_DAY", "-N CLASS_DAY"        move N class days (days_of_week, minus
#                                         non-instructional days)
#   "Fri NEAREST_CALENDAR_DAY"            move to the closest Fri, earlier or later
#                                         (Mon Tue Wed Thu Fri Sat Sun)
#   "23:59 ABS_TIME"                      set the time of day, keep the date
# unlock_offset and lock_offset take the same offsets, or "NONE".
"""
